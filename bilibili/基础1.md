一、CSS
    ...
    3.隐藏元素的方法有哪些？
        ...
        在CSS中，clip-path属性允许你指定一个元素的可见区域。你可以使用这个属性来裁剪元素的形状，只显示元素的某个特定区域。clip-path可以使用基本形状（如圆形、椭圆形、矩形等）、SVG路径，或者是一个URL，指向一个SVG剪切路径。
    4.px和rem的区别是什么？
        px是像素，显示器上给我们呈现画面的像素，每个像素的大小是一样，绝对单位长度
        rem，相对单位，相对于html根节点的font-size的值，
        直接给html节点的font-size:62.5%;
            1rem = 10px; （16px*62.5%=10px）
         
         如果你将HTML根节点的font-size设置为62.5%，由于浏览器默认的font-size通常是16px，那么计算出来的font-size将会是10px（因为16px * 62.5% = 10px）。这样设置的好处是，1rem将等于10px，这使得将rem单位转换为px变得非常直观和简单。
    5.重绘重排有什么区别？
    ...
    7.CSS的哪些属性哪些可以继承？哪些不可以继承？
        ...
        z-index
    8.有没有用过预处理器？
       CSS 预处理器是一种脚本语言，它扩展了 CSS 的功能，并在编译过程中将其转换为正常的 CSS。预处理器提供了许多有用的特性，如变量、嵌套规则、混合（mixins）、函数、条件语句和循环，这些特性可以使 CSS 的编写更加高效和模块化。
        
        常见的 CSS 预处理器包括：

        Sass (SCSS): 提供了丰富的语法和功能，是最受欢迎的 CSS 预处理器之一。
        LESS: 类似于 Sass，但有自己的语法和特性。
        Stylus: 提供了非常灵活的语法和强大的功能，允许写得更少，做得更多。
        使用预处理器可以极大地提高开发效率，使得管理大型样式表变得更加容易。然而，它们也需要一个编译过程，将预处理器语言转换为浏览器可以理解的标准 CSS。
...
　　使用border-image每次都要去调整图片，可以借助于PostCSS的插件postcss-write-svg来帮助我们。
            5、box-shadow
            可以使用box-shadow来模拟边框
